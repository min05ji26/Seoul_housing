...
SLOT_QUESTIONS = {
    "work_address":      "직장 주소를 알려주세요. (예: 강남구 테헤란로 521)",
...
class ChatBot:
    ...
    def _missing_required(self) -> List[str]:
        ...
        for s in REQUIRED_SLOTS:
            if self.slots.get(s) is None:
                missing.append(s)
        if self.slots.get("rent_type") == "월세" and self.slots.get("monthly_manwon") is None:
            missing.append("monthly_manwon")
        return missing
    ...
    def process(self, user_text: str):
        ...
        if self._last_asked_slot and new_slots.get(self._last_asked_slot) is None:
            ...
            if asked == "work_address" and txt:
                ...
                m = re.search(r"(\d+)", txt)
                if m:
                    new_slots["allowed_minutes"] = int(m.group(1))
            elif asked == "deposit_manwon":
                m = re.search(r"(\d+)\s*억\s*(\d+)?\s*천?", txt)
                if m:
                    new_slots["deposit_manwon"] = int(m.group(1))*10000 + (int(m.group(2))*1000 if m.group(2) else 0)
                else:
                    m2 = re.search(r"(\d+)\s*억", txt)
                    if m2:
                        new_slots["deposit_manwon"] = int(m2.group(1))*10000
                    else:
                        m3 = re.search(r"(\d{3,})\s*만?", txt)
                        if m3:
                            new_slots["deposit_manwon"] = int(m3.group(1))
    ...
    def _build_summary(self) -> str:
        ...
        lines.append(f"- 우선순위: {s.get('weight_preference','균형')}")
        vibe = s.get("vibe")
        if vibe:
